...
func(cw *ClientWorker) createWidgets() {
	...
	cw.win.Add(mainBox)
	mainBox.PackStart(setup, false, false, 0)
	mainBox.PackStart(start, false, false, 0)
	mainBox.PackStart(pause, false, false, 0)
	mainBox.PackStart(teardown, false, false, 0)
...
func(cw *ClientWorker) listenRtp()  {
	for {
		...
		var reqLen int
		var err error
		reqLen, cw.cAddr, err = cw.rtpSocket.ReadFromUDP(buf)
		fmt.Println(cw.cAddr.String(), "HElllo")
		if err != nil {
			fmt.Println("Didn`t receive data!")
			//cw.event.Done()
			if cw.teardownAcked == 1 {
				_ = cw.rtpSocket.Close()
				break
			}
			fmt.Println("Error reading:", err.Error())
		}
		fmt.Println("Data recieved check", reqLen, buf[:reqLen])
		if reqLen > 0 {
...
func(cw *ClientWorker) updateMovie(imageFile string) {
	...
	mainBox.PackStart(image, false, false, 0)
